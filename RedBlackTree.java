...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /**
   * Resolves a red child under a red parent red black tree property violations
  ...
  private void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
    ...
    try {
      if (newNode == null || newNode.parent == null)
        uncle = null;
      else if (newNode.parent.equals(newNode.parent.parent.leftChild)) {
    ...
    if (newNode != null && newNode != root && newNode.parent.isBlack == false) {
      // if uncle is red
      ...
      else if (newNode.parent.equals(newNode.parent.parent.rightChild)) {
  ...
  /**
   * Returns the Node searched for
   * @param root
   * @param points
   * @return Node of searched value
   */
  public Node<PlayerInterface> search(Node<PlayerInterface> root, int points)
  {
      // Base Cases: root is null or key is present at root
      if (root==null || root.data.getPoints()==points)
          return root;
   
      // Key is greater than root's key
      if (root.data.getPoints() < points)
         return search(root.rightChild, points);
   
      // Key is smaller than root's key
      return search(root.leftChild, points);
  }
